...
def run() -> dict:
    ...
    try {
      $events = Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4625} -MaxEvents 3
      $events | Select-Object TimeCreated, Id, ProviderName | ConvertTo-Json -Depth 3
    } catch {
      # Let stderr carry the "No events found" text; python transform handles it as []
      throw
    }
    """)
    ...
    try {
      $events = Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4624} -MaxEvents 3
      $events | Select-Object TimeCreated, Id, ProviderName | ConvertTo-Json -Depth 3
    } catch {
      throw
    }
